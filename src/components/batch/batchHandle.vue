    ...
    <div class="batchModel">
        <Row>
            <Form ref="searchForm" :model="searchForm" inline :label-width="80" class="search-form">
                ...
                <!-- <Form-item label="书记员" prop="clerk>
                ...
                <Form-item label="法官助理" prop="selassistant>
                ...
                <!-- <Form-item label="案号分配" prop="caseno>
                ...
                <Form-item label="适用程序" prop="Procedure">
                    <Select v-model="searchForm.Procedure" clearable filterable placeholder="请选择" @on-change="selProcedure" style="width: 160px">
                ...
                <Form-item label="起始案号" >
                    <Form ref="contrTypeTabList" :model="contrTypeTabList" inline>
                        <Form-item>
                            <Input placeholder="例：(年份)闽(0203)民初" class="reg_groupSelect reg_groupSelectwidth" v-model="contrTypeTabList.numFormat" @on-keyup="numFormatChange"></Input>
                        </Form-item>
                        <Form-item prop="startCaseNo">
                            <Input placeholder="自然数" class="reg_groupSelect" v-model="contrTypeTabList.startCaseNo" @on-keyup="numFormatChange1"></Input>
                        </Form-item>
                        <Form-item>
                            <span>--</span>
                        </Form-item>
                        <Form-item prop="endCaseNo">
                            <Input placeholder="自然数" class="reg_groupSelect" v-model="contrTypeTabList.endCaseNo" :disabled="true"></Input>
                        </Form-item>
                        <Form-item>
                            <span>号</span>
                        </Form-item>
        ...
        <div class='button-clas' style='text-align:center;'>
            <Button size='large' :loading="buttonLoading" @click="save" style="background:#2083D8;color:#fff;font-size:16px;width:128px;" >确认立案</Button>
        ...
        <div class="noneShow" v-show="modal3" style="width:200px;position: absolute;top: 20px;z-index: 99999;left: 50%;margin-left: -100px;background: #fff;border-radius: 15px;">
        ...
        <div class="noneShow" v-show="modal2" style="width:200px;position: absolute;top: 20px;z-index: 99999;left: 50%;margin-left: -100px;background: #fff;border-radius: 15px;">
            <div class="ivu-modal-header">
...
export default {
            ...
            columns:[
                            ...
                            h('Input', {
                                ...
                                on: {
                                    ...
                                    'on-blur': (event) => {
                                        ...
                                        if(event.target.value == this.listData[params.index].applyStandard || event.target.value == '' || event.target.value <= 0){
                                            return false;
                                        }
                                        this.listData[params.index].applyStandard = event.target.value;
                                        let ary = [];
                                        const data = {
                                            type:1,
                                            lawCaseId:this.listData[params.index].lawCaseId.toString(),
                                            basicCost:'0.0',
                                            property:'0.0',
                                            applyStandard:this.listData[params.index].applyStandard.toString(),
                                        }
                                        ary.push(data)
                                        let ds = {
                                            lawCaseList:ary,
                                        ...
                                        litigationFeeCalculation(ds).then(res => {
                                            ...
                                            if(res.data.state == 100){
                                                ...
                                                this.listData[params.index].litigationCost = list[0].discount;
                                            }
                                            Vue.set(this.listData, params.index, this.listData[params.index]);
                                        })
                                    }
                                }
                            }),
                                ...
                                h('span',
                                    {
                                        ...
                                        on: {
                                            click: () => {
                                                ...
                                                this.searchForm2.briefs = this.selRow.briefId ? this.selRow.briefId : "";
                                                console.log(this.modal2back)
                                                // return false;
                                            }
                                        },
                                    },
                                    this.listData[params.index].briefName ? this.listData[params.index].briefName : "请选择"
                                ),
                            ]
                                ...
                                h('span',
                                    {
                                        ...
                                        on: {
                                            click: () => {
                                                ...
                                                this.modal2back = true;
                                                this.selRow = this.listData[params.index];
                                                this.searchForm2.mkks = this.selRow.courtId ? this.selRow.courtId : "";
                    ...
                    render: (h, params) => {
                        return h('Input', {
                            ...
                            on: {
                                'on-focus': () => {
                                    this.$nextTick(()=>{                       
                                        document.getElementById('rwmca'+params.index).childNodes.forEach(item=>{
                                            if(item.tagName == 'INPUT'){
                                                item.focus();
                                            }
                                        })
                        ...
                        return h('div',{
                            ...
                            h('Input', {
                                ...
                                on: {
                                    'on-focus': () => {
                                        this.$nextTick(()=>{                       
                                            document.getElementById('rwmcs'+params.index).
                                            childNodes.forEach(item=>{
                                                if(item.tagName == 'INPUT'){
                                                    item.focus();
                                                }
                ...
                },
                {
                    ...
                    render: (h, params) => {
                        return h('Select', {
                            props:{
                                value: this.searchForm.Procedure,
                                filterable:'filterable',
                                clearable:'clearable',
                                transfer:'transfer'
                            },
    ...
    methods:{
        ...
        numFormatChange(){//去除案号格式输入填写空格
            this.contrTypeTabList.numFormat=delSpace(this.contrTypeTabList.numFormat,"g");
        },
        numFormatChange1(){//去除起始案号填写空格并设置相关数据
            this.contrTypeTabList.startCaseNo=delSpace(this.contrTypeTabList.startCaseNo,"g");//设置起始案号
            if(this.contrTypeTabList.startCaseNo){//判断起始案号非空
                if(!this.isDown){//判断是否倒序
                    //正序
                    this.contrTypeTabList.endCaseNo=Number(this.contrTypeTabList.startCaseNo)+this.listDatas.length-1//设置结束案号
                }else{
                    //倒序
                    this.contrTypeTabList.endCaseNo=Number(this.contrTypeTabList.startCaseNo)-this.listDatas.length+1//设置结束案号
                }
                ...
                this.listData.forEach((item,index) => {
                    item.caseNo=this.contrTypeTabList.numFormat+nowNum.toString()+"号"//拼接
           ...
           //获取法院部门列表
            ...
            for(let i=0;i<this.listData.length;i++){
                const data = {
                    ...
                    basicCost:'0.0',
                    property:'0.0',
                    applyStandard:this.listData[i].applyStandard ? this.listData[i].applyStandard.toString() : '0.0',
                }
                ary.push(data)
            }
            let ds = {
