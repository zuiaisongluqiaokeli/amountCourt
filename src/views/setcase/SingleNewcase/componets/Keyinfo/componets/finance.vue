  ...
  <div>
        ...
        <Panel  v-for="(item, key) in keyList.creditContract" :key="'info'+key" :name="'info'+key">
          ...
          <div slot="content" class="ft-plant-div2">
            <Form :model="item" label-position="right" :label-width="190" :ref="'info' + key" style="margin:0 auto">
              <Tooltip content="删除当前合同" @click.native="delcontract('creditContract','openPandel',item)" class="myTooltip" placement="bottom">
              ...
              <FormItem label="是否诉请解除该授信合同" class="ft_form ft-plant-chooseType" prop="terminateContract">
              ...
              <FormItem label="授信金额（元）" class="ft_form ft-plant-chooseType" prop="money">
                <Input  v-model="item.money" @on-blur="onNumberChange($event,item,'money')" placeholder="请输入授信金额" ></Input>
              </FormItem>
            </Form>
          </div>
        </Panel>
      ...
      <Form label-position="right" style="margin:0 auto;width: 441px;margin-top:15px">
        <FormItem>
          <div class='ft_public1'>
            <a href="javascript:void(0)" style="" class="ft_public2 myaddBtn" @click="addcontract('creditContract','credidList')">增加授信合同</a>
          </div>
        </FormItem>
      </Form>
    </div>
    ...
    <div v-if="sw2==true">
      <Collapse simple v-model="openPandel1">
        <Panel  v-for="(item, key) in keyList.loanContract" :key="'info1'+key" :name="'info1'+key">
          ...
          <div slot="content" class="ft-plant-div2">
            <Form :model="item" label-position="right" :label-width="190" :ref="'info1' + key" style="margin:0 auto">
              <Tooltip content="删除当前合同" @click.native="delcontract('loanContract','openPandel1',item)" class="myTooltip" placement="bottom">
                <Icon type="ios-close"  class="myCloseIco"/>
              </Tooltip>
              <FormItem label="借款合同名称" class="ft_form ft-plant-chooseType" prop="name">
                <Input v-model="item.name" placeholder="请输入借款合同名称"></Input>
              </FormItem>
              <FormItem label="借款人" class="ft_form ft-plant-chooseType" prop="borrowPerson">
                <Input v-model="item.borrowPerson" placeholder="请输入借款人"></Input>
              </FormItem>
              <FormItem label="是否诉请解除该借款合同" class="ft_form ft-plant-chooseType" prop="terminateContract">
                <RadioGroup v-model="item.terminateContract">
                  <Radio label="是"></Radio>
                  <Radio label="否"></Radio>
                </RadioGroup>
              ...
              <FormItem label="借款本金（元）" class="ft_form ft-plant-chooseType" prop="money">
                <Input v-model="item.money" @on-blur="onNumberChange($event,item,'money')" placeholder="请输入借款本金"></Input>
              </FormItem>
              <FormItem label="借款期限" class="ft_form ft-plant-chooseType" prop="term">
                <Input v-model="item.term" placeholder="请输入借款期限"></Input>
              </FormItem>
              <FormItem label="还款方式" class="ft_form ft-plant-chooseType ft-plant-slectType" prop="repaymentType">
                <Input v-model="item.repaymentType" placeholder="请输入还款方式"></Input>
              </FormItem>
              <FormItem label="借款利率" class="ft_form ft-plant-chooseType ft-plant-slectType" prop="interestRate">
                <Input v-model="item.interestRate" placeholder="请输入借款利率"></Input>
              </FormItem>
              <FormItem label="罚息利率" class="ft_form ft-plant-chooseType ft-plant-slectType" prop="penaltyInterestRate">
                <Input v-model="item.penaltyInterestRate" placeholder="请输入罚息利率"></Input>
              </FormItem>
              <FormItem label="复利利率" class="ft_form ft-plant-chooseType ft-plant-slectType" prop="compoundInterestRate">
                <Input v-model="item.compoundInterestRate" placeholder="请输入复利利率"></Input>
              </FormItem>
              <FormItem label="罚息、复利约定" class="ft_form ft-plant-chooseType" prop="penaltyAppointment">
                <Input v-model="item.penaltyAppointment" placeholder="请输入罚息、复利约定"></Input>
              </FormItem>
              <FormItem label="解除合同或提前收贷的约定" class="ft_form ft-plant-chooseType" prop="advanceRelieveAppointment">
                <Input v-model="item.advanceRelieveAppointment" placeholder="请输入解除合同或提前收贷的约定"></Input>
              </FormItem>
              <FormItem label="违规金约定" class="ft_form ft-plant-chooseType" prop="breakContractAppointment">
                <Input v-model="item.breakContractAppointment" placeholder="请输入违规金约定"></Input>
              </FormItem>
              <FormItem label="实现债券费用的约定" class="ft_form ft-plant-chooseType" prop="debtExpenseAppointment">
      ...
      <Form label-position="right" style="margin:0 auto;width: 441px;margin-top:15px">
        <FormItem>
          <div class='ft_public1'>
            <a href="javascript:void(0)" style="" class="ft_public2 myaddBtn" @click="addcontract('loanContract','loanList')">增加借款合同</a>
          </div>
        </FormItem>
      </Form>
    </div>
    ...
    <div v-if="sw3==true">
      <Collapse simple v-model="openPandel2">
        <Panel  v-for="(item, key) in keyList.ensureContract" :key="'info2'+key" :name="'info2'+key">
          ...
          <div slot="content" class="ft-plant-div2">
            <Form :model="item" label-position="right" :label-width="190" :ref="'info2' + key" style="margin:0 auto">
              <Tooltip content="删除当前合同" @click.native="delcontract('ensureContract','openPandel2',item)" class="myTooltip" placement="bottom">
                <Icon type="ios-close"  class="myCloseIco"/>
              </Tooltip>
      ...
      <Form label-position="right" style="margin:0 auto;width: 441px;margin-top:15px">
        <FormItem>
          <div class='ft_public1'>
            <a href="javascript:void(0)" style="" class="ft_public2 myaddBtn" @click="addcontract('ensureContract','ensureList')">增加保证合同</a>
          </div>
        </FormItem>
      </Form>
    </div>
    ...
    <div v-if="sw4==true">
      <Collapse simple v-model="openPandel3">
        <Panel  v-for="(item, key) in keyList.mortgageContract" :key="'info3'+key" :name="'info3'+key">
          抵押合同（{{item.name ? item.name :'暂无名称'}}）
          <div slot="content" class="ft-plant-div2">
            <Form :model="item" label-position="right" :label-width="190" :ref="'info3' + key" style="margin:0 auto">
              <Tooltip content="删除当前合同" @click.native="delcontract('mortgageContract','openPandel3',item)" class="myTooltip" placement="bottom">
                <Icon type="ios-close"  class="myCloseIco"/>
              </Tooltip>
      ...
      <Form label-position="right" style="margin:0 auto;width: 441px;margin-top:15px">
        <FormItem>
          <div class='ft_public1'>
            <a href="javascript:void(0)" style="" class="ft_public2 myaddBtn" @click="addcontract('mortgageContract','pawnList')">增加抵押合同</a>
          </div>
        </FormItem>
      </Form>
    </div>
    ...
    <div v-if="sw5==true">
      <Collapse simple v-model="openPandel4">
        <Panel  v-for="(item, key) in keyList.impawnContract" :key="'info4'+key" :name="'info4'+key">
          ...
          <div slot="content" class="ft-plant-div2">
            <Form :model="item" label-position="right" :label-width="190" :ref="'info4' + key" style="margin:0 auto">
              <Tooltip content="删除当前合同" @click.native="delcontract('impawnContract','openPandel4',item)" class="myTooltip" placement="bottom">
                <Icon type="ios-close"  class="myCloseIco"/>
              </Tooltip>
      ...
      <Form label-position="right" style="margin:0 auto;width: 441px;margin-top:15px">
        <FormItem>
          <div class='ft_public1'>
            <a href="javascript:void(0)" style="" class="ft_public2 myaddBtn" @click="addcontract('impawnContract','impawnList')">增加抵押合同</a>
          </div>
        </FormItem>
      </Form>
    </div>
    ...
    <FormItem label="欠款本金（元）" class="ft_form ft-plant-chooseType" prop="deptCapital">
      <Input v-model="keyList.deptCapital" @on-blur="onNumberChange($event,keyList,'deptCapital')" placeholder="请输入欠款本金"></Input>
    </FormItem>
    <FormItem label="利息（元）" class="ft_form ft-plant-chooseType" prop="interest">
      <Input v-model="keyList.interest" @on-blur="onNumberChange($event,keyList,'interest')" placeholder="请输入利息"></Input>
    </FormItem>
    <FormItem label="罚息（元）" class="ft_form ft-plant-chooseType" prop="penaltyInterest">
      <Input v-model="keyList.penaltyInterest" @on-blur="onNumberChange($event,keyList,'penaltyInterest')" placeholder="请输入罚息"></Input>
    </FormItem>
    <FormItem label="复利（元）" class="ft_form ft-plant-chooseType" prop="compoundInterest">
      <Input v-model="keyList.compoundInterest" @on-blur="onNumberChange($event,keyList,'compoundInterest')" placeholder="请输入复利"></Input>
    </FormItem>
    <FormItem label="最新欠款利息、罚息、复利（元）" class="ft_form ft-plant-chooseType" prop="newlyInterest">
      <Input v-model="keyList.newlyInterest" @on-blur="onNumberChange($event,keyList,'newlyInterest')" placeholder="请输入最新欠款利息、罚息、复利"></Input>
    </FormItem>
    <FormItem label="最新欠款利息等（元）" class="ft_form ft-plant-chooseType" prop="newArrearsInterest">
      <Input v-model="keyList.newArrearsInterest" @on-blur="onNumberChange($event,keyList,'newArrearsInterest')" placeholder="请输入最新欠款利息等"></Input>
    </FormItem>
    <FormItem label="违约金金额（元）" class="ft_form ft-plant-chooseType" prop="breakContractMoney">
      <Input v-model="keyList.breakContractMoney" @on-blur="onNumberChange($event,keyList,'breakContractMoney')" placeholder="请输入违约金金额"></Input>
    </FormItem>
    <FormItem label="实现债权的费用（元）" class="ft_form ft-plant-chooseType" prop="debtExpense">
      <Input v-model="keyList.debtExpense" @on-blur="onNumberChange($event,keyList,'debtExpense')" placeholder="请输入实现债权的费用"></Input>
    </FormItem>
    <FormItem label="律师费（元）" class="ft_form ft-plant-chooseType" prop="lawyerFees">
      <Input v-model="keyList.lawyerFees" @on-blur="onNumberChange($event,keyList,'lawyerFees')" placeholder="请输入律师费"></Input>
    </FormItem>
    <FormItem label="保全费（元）" class="ft_form ft-plant-chooseType" prop="preservationFees">
      <Input v-model="keyList.preservationFees"  @on-blur="onNumberChange($event,keyList,'preservationFees')" placeholder="请输入保全费"></Input>
    </FormItem>
    <FormItem label="公告费（元）" class="ft_form ft-plant-chooseType" prop="announcementFees">
      <Input v-model="keyList.announcementFees" @on-blur="onNumberChange($event,keyList,'announcementFees')" placeholder="请输入公告费"></Input>
    </FormItem>
    <FormItem label="债权其他费用（元）" class="ft_form ft-plant-chooseType" prop="claimsOtherFees">
      <Input v-model="keyList.claimsOtherFees" @on-blur="onNumberChange($event,keyList,'claimsOtherFees')" placeholder="请输入债权其他费用"></Input>
    </FormItem>
    ...
    <FormItem label="财产保全金额（元）" class="ft_form ft-plant-chooseType" prop="propertyPreservationMoney">
      <Input v-model="keyList.propertyPreservationMoney" @on-blur="onNumberChange($event,keyList,'propertyPreservationMoney')" placeholder="请输入财产保全金额"></Input>
    </FormItem>
    <FormItem label="财产保全费（元）" class="ft_form ft-plant-chooseType" prop="propertyPreservationFees">
      <Input v-model="keyList.propertyPreservationFees" @on-blur="onNumberChange($event,keyList,'propertyPreservationFees')" placeholder="请输入财产保全费"></Input>
    </FormItem>
    ...
    <FormItem label="请求支付的事实与理由" class="ft_form ft-plant-chooseType" prop="requestPayReason">
      <Input v-model="keyList.requestPayReason" placeholder="请输入请求支付的事实与理由"></Input>
    </FormItem>
    <FormItem label="申请支付金额（元）" class="ft_form ft-plant-chooseType" prop="requestPayMoney">
      <Input v-model="keyList.requestPayMoney"  @on-blur="onNumberChange($event,keyList,'requestPayMoney')" placeholder="请输入申请支付金额"></Input>
    </FormItem>
    ...
    <FormItem label="支付令申请费（元）" class="ft_form ft-plant-chooseType" prop="paymentOrderFees">
      <Input v-model="keyList.paymentOrderFees" @on-blur="onNumberChange($event,keyList,'paymentOrderFees')" placeholder="请输入支付令申请费"></Input>
    </FormItem>
    ...
    <FormItem label="终结督促程序申请费（元）" class="ft_form ft-plant-chooseType" prop="endSupervisionFees">
      <Input v-model="keyList.endSupervisionFees" @on-blur="onNumberChange($event,keyList,'endSupervisionFees')" placeholder="请输入终结督促程序申请费"></Input>
    </FormItem>
    <FormItem label="终结督促程序裁定适用法条" class="ft_form ft-plant-chooseType" prop="endSupervisionLaw">
      <Input v-model="keyList.endSupervisionLaw" placeholder="请输入终结督促程序裁定适用法条"></Input>
    </FormItem>
    ...
    <FormItem label="申请办理信用卡的时间" class="ft_form ft-plant-chooseType" prop="creditCardApplyDate">
      <DatePicker
        type="date"
        :editable="false"
        placeholder="请选择申请办理信用卡的时间"
        format="yyyy年MM月dd日"
        style="width: 300px"
        @on-change="ft_keyinfo_chgTime(keyList,'creditCardApplyDate',$event)"
      ></DatePicker>
    </FormItem>
  </div>
</template>
