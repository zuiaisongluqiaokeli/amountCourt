            ...
            <Form ref="userForm" :model="userForm" :label-width="70" :rules="userFormValidate">
                ...
                <FormItem label="角色分配" prop="roles">
                  <Select v-model="userForm.roles" multiple>
                      <Option v-for="item in roleList" filterable :value="item.id" :key="item.id" :label="item.name">
...
export default {
  ...
  methods: {
    ...
    getUserList() {
      // 多条件搜索用户列表
      this.loading = true;
      getInsiders(this.searchForm).then(res => {
        this.loading = false;
        if(res.data.state == 100){
          this.data = res.data.insiderList;
    ...
    submitUser() {
      ...
      for(let i=0;i<this.userForm.roles.length;i++){
        if(i == this.userForm.roles.length - 1){
          str = str + this.userForm.roles[i]
        }else{
          str = str + this.userForm.roles[i] + ',';
        }
      }
      ...
      editInsiderInfo(this.userId,str).then(res => {
        if(res.data.state == 100){
    ...
    edit(v) {
      ...
      console.log(v)
      getInsiderInfo(v.idCard).then(res => {
        if(res.data.state == 100){
           this.userForm.roles =[];
